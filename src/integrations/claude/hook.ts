...
async function main(): Promise<void> {
  const handler = process.argv[2] as HandlerName | undefined;
  if (!handler || !(handler in DEADLINES)) {
    // Not an error worth surfacing: exit quietly so a stale hook config from
    // an older version cannot spam the user.
    process.exit(0);
  }

  const input = parseHookInput(await readStdin());

  const output = await withDeadline(runHandler(handler, input), DEADLINES[handler], '');
...
async function onSessionStart(input: HookInput): Promise<string> {
  ...
  if (input.session_id) {
    const existing = await readSessionRecord(workspace.paths, input.session_id);
    await writeSessionRecord(
      workspace.paths,
      existing
        ? { ...existing, last_activity_at: now(), cwd }
        : newSessionRecord(input.session_id, cwd, {
...
async function onStop(input: HookInput): Promise<string> {
  ...
  if (record.project_id === null) {
    const { project } = await registry.identify(cwd, workspace.machineId);
    record.project_id = project?.id ?? null;
  }
  ...
  await writeSessionRecord(workspace.paths, record);
  return '';
}
...
async function onPreCompact(input: HookInput): Promise<string> {
  ...
  record.project_id = project?.id ?? record.project_id;
...
async function onPostCompact(input: HookInput): Promise<string> {
  ...
  if (record && input.session_id) {
    await writeSessionRecord(workspace.paths, {
      ...record,
      checkpointed: true,
...
async function onSessionEnd(input: HookInput): Promise<string> {
  ...
  // A session with a single turn and no repository change is someone opening
  // Claude Code and closing it again. There is nothing to remember.
  ...
  await writeSessionRecord(workspace.paths, { ...record, checkpointed: true });
  return '';
}
