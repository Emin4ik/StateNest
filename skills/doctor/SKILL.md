---
name: doctor
description: Diagnose Project Brain itself when it seems not to be working — no context appearing at session start, checkpoints not saving, a project not being recognised. Use when the user reports Project Brain misbehaving.
---

# Diagnose Project Brain

1. Run `pb doctor` with the Bash tool and show the user the result.
...
2. If the user's current project is not being recognised, call
   ...
   - "No registered project" with a git repository present → they need `pb add .`
   - No git repository → confirm which directory they expect to be the project

3. If `pb` is not on the PATH at all, Project Brain is not installed globally.
   The fix is `npm install -g project-brain`, then `pb init`.
...
- If `pb doctor` reports "Plugin build: compiled output missing", the plugin was
